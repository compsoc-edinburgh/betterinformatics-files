...
class TestExistingAnswer(ComsolTestWithData):
    ...
    def test_expertvote_nonexpert(self):
        answer = self.answers[1]
        self.post('/api/exam/setexpertvote/{}/'.format(answer.id), {'vote': True}, status_code=403)
...
class TestNonexisting(ComsolTestWithData):
    ...
    def test_set_answer(self):
        ...
        self.assertFalse(Answer.objects.filter(answer_section=self.mysection, author=self.get_my_user()).exists())
        ...
        self.assertTrue(Answer.objects.filter(answer_section=self.mysection, author=self.get_my_user()).exists())
