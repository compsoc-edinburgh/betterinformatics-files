...
# Originally by Joshua Tauberer (https://joshdata.me).
# License: CC0 1.0 Universal (Public Domain Dedication)
# https://github.com/JoshData/pdf-redactor/blob/primary/pdf_redactor.py
#
# Modifications:
# 2026/04/21 by Yuto Takano with the help of GitHub Copilot.
#   Fixed some multi-byte CJK characters disappearing after going through the
            ...
            def serializer(xml_root):
                ...
                if hasattr(xml.etree.ElementTree, "register_namespace"):
                    ...
                    xml.etree.ElementTree.register_namespace(
                        "pdf13", "http://ns.adobe.com/pdf/1.3/"
                    )
                    xml.etree.ElementTree.register_namespace(
...
def build_text_layer(document, options):
    ...
    class TextToken:
        ...
        def __init__(self, token, font):
            self.font = font
            # Keep the original serialized token text for lossless round-tripping
            # when this token's value is not changed.
            self.raw_original_token = str(token)
            self.raw_original_value = token.to_bytes()
            self.original_value = toUnicode(self.raw_original_value, font, fontcache)
            self.value = self.original_value
            self.is_text_token = True
        ...
        def __str__(self):
            ...
            if self.value == self.original_value:
                ...
                return self.raw_original_token
            else:
    ...
    for page in document.pages:
        ...
        def make_mutable_string_token(token):
            if isinstance(token, PdfString):
                token = TextToken(token, current_font)
                ...
                if current_font and current_font.BaseFont:
                    fontcache.setdefault(current_font.BaseFont, set()).update(
                        token.value
                    )
            return token
        ...
        for token in tokenize_streams([cmap.stream]):
            ...
            if token == "def":
                ...
                for code1, code2, cid_or_name1 in chunk_triples(operand_stack):
                    if not isinstance(code1, PdfString) or not isinstance(
                        code2, PdfString
                    ):
                        continue
        ...
        for m in pattern.finditer(text_content):
            ...
            while i1 < i2:
                ...
                while (
                    ...
                    and text_tokens_charpos
                    + len(text_tokens[text_tokens_index].value)
                    - text_tokens_token_xdiff
                    <= i1
                ):
                    text_tokens_charpos += (
                        len(text_tokens[text_tokens_index].value)
                ...
                # Do the replacement.
                tok.value = (
                    tok.value[: mpos + text_tokens_token_xdiff]
                    + r
                    + tok.value[mpos + mlen + text_tokens_token_xdiff :]
                )
...
def apply_updated_text(document, text_tokens, page_tokens):
    ...
    def token_has_changes(tok):
        if hasattr(tok, "is_text_token"):
            return tok.value != tok.original_value
        if isinstance(tok, PdfArray):
            return any(token_has_changes(x) for x in tok)
        if isinstance(tok, InlineImage):
            return any(
                token_has_changes(x) or token_has_changes(y) for x, y in tok.items()
            )
    ...
    for i, page in enumerate(document.pages):
        ...
        # Avoid touching page streams that do not contain redaction changes.
        if not any(token_has_changes(tok) for tok in page_tokens[i]):
            continue
