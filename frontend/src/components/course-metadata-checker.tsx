...
import { Anchor, Button, Group, LoadingOverlay, Select, Table } from "@mantine/core";
...
const loadEuclidList = async () => {
  return (await fetchGet("/api/category/listeuclidcodes/")).value as { "code": string, "category": string }[];
}
...
const CourseEuclidAssociateButton: React.FC<CourseEuclidAssociateButtonProps> = ({ code, onAssociated, categories }) => {
...
const CourseMetadataChecker: React.FC = () => {
  ...
  const courseCheckLoading = biLoading || euclidListLoading || categoriesLoading;
  ...
  const missingMapping = useMemo(() => {
    ...
    for (const biCourse of Object.values(biData)) {
      ...
      if (euclid_shadow && !euclidList.find(mapping => mapping.code === euclid_shadow)) {
        missingMapping.push({ "code": euclid_shadow, "data": biCourse, "loading": false });
      }
    }
    ...
    <>
      ...
      <div>
        ...
        <Table striped fontSize="md">
          ...
          <tbody>
            {missingMapping && missingMapping.map(({ code, data, loading }) => (
              <tr key={code}>
                <td>{code}</td>
                <td>
                  {data.name}<br />
                  {`${data.delivery} / SCQF ${data.level} / ${data.credits} credits`}<br />
                  <Anchor
                    color="blue"
                    component="a"
                ...
                <td>
                  <CourseEuclidAssociateButton code={code} onAssociated={reloadEuclidList} categories={categories}/>
                </td>
              </tr>
            ))}
          </tbody>
